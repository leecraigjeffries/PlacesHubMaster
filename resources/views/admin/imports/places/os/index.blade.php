...
@section('title')
    {!! Breadcrumbs::view('app._title', 'admin.imports.places.os.index') !!}
@endsection

@section('breadcrumbs')
    {!! Breadcrumbs::render('admin.imports.places.os.index') !!}
@endsection

@section('app.content')

    <form method="GET" action="{{ route('admin.imports.places.os.index') }}">
        <fieldset id="filter">
            ...
            <div class="row">
                <div class="col text-center">
                    ...
                    <a href="{{ route('admin.imports.places.os.index') }}">Clear</a>
                </div>
            </div>
        </fieldset>
    </form>
    ...
    <table class="table table-striped" id="table-os-places">
        ...
        <tr>
            @foreach($osSearch->getHeadingsTranslated() as $order_by => $headingText)
                <th scope="col" @if($order_by === $osSearch->getOrderBy()) class="active" @endif>
                    <a href="{{ route('admin.imports.places.os.index', $osSearch->getAppends([
                    'order_by' => $order_by,
        ...
        @foreach($results as $result)
            <tr>
                <td>
                    <a href="{{ route('admin.imports.places.os.show', $result) }}">{{ $result->name }}</a>
                    <ul>
                        @foreach($result->getAdminTypes() as $type)
                            @if($result->$type && $result->id !== $result->$type->id)
                                <li>
                                    <a href="{{ route('admin.imports.places.os.show', $result->$type) }}">{{ $result->$type->name }}</a>
                                </li>
                            @endif
                        @endforeach
                    </ul>
                ...
                <td>
                    <a href="{{ route('admin.imports.places.os.index', ['os_type' => $result->os_type]) }}">{{ $result->os_type }}</a>
                </td>
            </tr>
        @endforeach
        </tbody>
