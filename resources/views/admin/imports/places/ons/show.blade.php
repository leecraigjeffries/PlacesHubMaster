...
@section('title')
    {!! Breadcrumbs::view('app._title', 'admin.imports.places.ons.show', $onsPlace) !!}
@endsection

@section('breadcrumbs')
    {!! Breadcrumbs::render('admin.imports.places.ons.show', $onsPlace) !!}
@endsection
...
@section('app.content')
        ...
        <div class="col">
            <dl>
                @if($onsPlace->ons_id)
                    ...
                    <dd>
                        <a href="{{ route('admin.imports.places.ons.show-ons-id', [$onsPlace->ons_id]) }}">{{ $onsPlace->ons_id }}</a>
                        <a href="http://statistics.data.gov.uk/doc/statistical-geography/{{ $onsPlace->ons_id }}"
                ...
                <dd>
                    <a href="{{ route('admin.imports.places.ons.index', ['ons_type' => $onsPlace->ons_type]) }}">{{ $onsPlace->ons_type }}</a>
                </dd>
                @if($onsPlace->ipn_id)
                    <dt>@lang('placeshub.ipn_id')</dt>
                    <dd>
                        <a href="{{ route('admin.imports.places.ons.show-ipn-id', [$onsPlace->ipn_id]) }}">{{ $onsPlace->ipn_id }}</a>
                    </dd>
                @endif
            </dl>
                ...
                <div class="siblings">
                    <ul>
                        @foreach($otherIpnIds as $place)
                            <li>
                                <a href="{{ route('admin.imports.places.ons.show-ipn-id', [$place->ipn_id]) }}">{{ $place->ipn_id }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
    ...
    @foreach($onsPlace->childTypes() as $childType)
        @include('admin.imports.places.ons._data-table', ['type' => $childType, 'onsPlace' => $onsPlace])
    @endforeach
