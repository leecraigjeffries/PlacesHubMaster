...
@section('title')
    {!! Breadcrumbs::view('app._title', 'admin.imports.places.geo.index') !!}
@endsection

@section('breadcrumbs')
    {!! Breadcrumbs::render('admin.imports.places.geo.index') !!}
@endsection

@section('app.content')

    <form method="GET" action="{{ route('admin.imports.places.geo.index') }}">
        <fieldset id="filter">
            ...
            <div class="row">
                <div class="col text-center">
                    ...
                    <a href="{{ route('admin.imports.places.geo.index') }}">Clear</a>
                </div>
            </div>
        </fieldset>
    </form>
    ...
    <table class="table table-striped" id="table-geo-places">
        ...
        <tr>
            @foreach($geoSearch->getHeadingsTranslated() as $order_by => $headingText)
                <th scope="col" @if($order_by === $geoSearch->getOrderBy()) class="active" @endif>
                    <a href="{{ route('admin.imports.places.geo.index', $geoSearch->getAppends([
                    'order_by' => $order_by,
        ...
        @foreach($results as $result)
            <tr>
                <td>
                    <a href="{{ route('admin.imports.places.geo.show', $result) }}">{{ $result->name }}</a>
                    <ul>
                        @foreach($result->getAdminTypes() as $type)
                            @if($result->$type && $result->id !== $result->$type->id)
                                <li>
                                    <a href="{{ route('admin.imports.places.geo.show', $result->$type) }}">{{ $result->$type->name }}</a>
                                </li>
                            @endif
                        @endforeach
                    </ul>
                ...
                <td>
                    <a href="{{ route('admin.imports.places.geo.index', ['geo_type' => $result->geo_type]) }}">{{ $result->geo_type }}</a>
                </td>
                <td>
                    {{ $result->geo_code }}
                </td>
